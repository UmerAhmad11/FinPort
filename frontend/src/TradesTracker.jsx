          ...
          {loading ? (
            ...
            <ul className="trades-list">
              {trades.map((item, index) => (
                <li key={index} className={`trade-row ${item.type}`}>
                  <div className="trade-row-main">
                    {item.type === 'buy' && <span className="trade-icon">🟢</span>}
                    {item.type === 'sell' && <span className="trade-icon">🔴</span>}
                    {item.type === 'received' && <span className="trade-icon">🟡</span>}
                    <span className="trade-action-text">
                      {item.type === 'buy' && <>Bought <b>{item.quantity}</b> shares of <b>{item.stock_symbol.toUpperCase()}</b></>}
                      {item.type === 'sell' && <>Sold <b>{item.quantity}</b> shares of <b>{item.stock_symbol.toUpperCase()}</b></>}
                      {item.type === 'received' && <>Received <b>{item.quantity}</b> shares of <b>{item.stock_symbol.toUpperCase()}</b></>}
                    </span>
                  </div>
                  <div className="trade-row-userinfo">
                    {item.type === 'sell' && item.to && <span>to User <span className="trade-userid">{item.to}</span></span>}
                    {item.type === 'received' && item.from && <span>from User <span className="trade-userid">{item.from}</span></span>}
                  </div>
                </li>
              ))}
            </ul>
