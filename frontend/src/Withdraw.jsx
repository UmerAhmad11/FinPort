...
function Withdraw() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!withdrawAmount || isNaN(withdrawAmount) || Number(withdrawAmount) <= 0) {
          setResponse('⚠️ Please enter a valid withdrawal amount');
          return;
        }

        // Add client-side balance validation
        if (Number(withdrawAmount) > balance) {
          setResponse('❌ Insufficient balance for withdrawal');
          return;
        }
        ...
        try {
          ...
          if (!res.ok) {
            setResponse(data.detail || '❌ Withdrawal failed');
            return;
          }
      
          setBalance(data.new_balance);
          setResponse(`✅ Successfully withdrawn: $${withdrawAmount}`);
          setWithdrawAmount('');
        } catch (err) {
          console.error(err);
          setResponse('❌ Server error during withdrawal');
        }
      };
    ...
    return (
        <div className="frontpage-bg-fintech">
            ...
            <div className="trading-card">
                <h2 className="trading-card-title">Withdrawal Page for {loggedInUser}</h2>
          ...
          {error && (
            ...
            {response && (
                <div className="notification" style={{
                    ...
                    top: '20px',
                    right: '20px',
                    backgroundColor: response.startsWith('✅') ? '#4CAF50' : '#f44336',
                    color: 'white',
                    padding: '15px 25px',
                    borderRadius: '8px',
